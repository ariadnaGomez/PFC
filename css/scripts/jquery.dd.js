...
function dd(element, settings) {
	...
	var _createChild = function (opt) {
		...
		var prefix = _createElement("span", {
			className: _styles.label
		}, element[0]);
                var country = _createElement("span", {
			className: _styles.label
		}, element[1]);
		//checkbox
	...
	var _adjustOpen = function () {
		...
		if ((wH + st) < Math.floor(cH + mH + pos.top) || _settings.openDirection.toLowerCase() == 'alwaysup') {
			...
			$("#" + childid).css({ zIndex: _settings.zIndex});
	...
	var _updateTitleUI = function (index, byvalue) {
		...
		if (index == -1) {
			...
			value = byvalue;
                ...
                var element = value.text.split(' ');
		//update title and current
		$("#" + titleid).find("." + _styles.label).html(element[0]);
