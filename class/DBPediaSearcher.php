        ...
        $queryString = '
            SELECT DISTINCT * WHERE 
            {
               <' . $author . '> foaf:name ?name .
                ...
                OPTIONAL  { 
                        <' . $author . '> dbpprop:placeOfBirth ?birthPlace.
                        FILTER(langMatches(lang(?birthPlace), "EN"))}       
                OPTIONAL   {
                        <' . $author . '> dbpprop:birthPlace ?birthPlace2.                             
                         FILTER(langMatches(lang(?birthPlace2), "EN"))} 
                OPTIONAL   {
                        <' . $author . '> dbpedia-owl:birthPlace ?x.  
                        ?x rdfs:label ?birthPlace3                
                        FILTER(langMatches(lang(?birthPlace3), "EN")) }
                      
                
                
                OPTIONAL  {   
                        <' . $author . '> dbpprop:placeOfDeath ?deathPlace.
                        FILTER(langMatches(lang(?deathPlace), "EN"))}    
                OPTIONAL  {         
                        <' . $author . '> dbpprop:deathPlace ?deathPlace2.                             
                         FILTER(langMatches(lang(?deathPlace2), "EN"))}   
                OPTIONAL  {  
                        <' . $author . '> dbpedia-owl:deathPlace ?y.  
                        ?y rdfs:label ?deathPlace3                
                        FILTER(langMatches(lang(?deathPlace3), "EN")) } 
                           

                OPTIONAL {
                        <' . $author . '> dbpedia-owl:birthDate ?birthDate.}              
                OPTIONAL { 
                        <' . $author . '> dbpprop:birthDate ?birthDate2.}      
                                

                OPTIONAL { 
                        <' . $author . '> dbpedia-owl:deathDate ?deathDate. } 
                 OPTIONAL { 
                        <' . $author . '> dbpprop:deathDate ?deathDate2.}
